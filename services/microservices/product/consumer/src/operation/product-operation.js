...
import { productBackupDbWrite } from "../utils/productOutboxWrite.js";
...
export const productBulkInsert = async (events) => {
...
export const productBulkUpdate = async (events) => {
  if (!Array.isArray(events)) {
    events = [events];
  }
...
export const productBulkDelete = async (events) => {
  if (!Array.isArray(events)) {
    events = [events];
  }
